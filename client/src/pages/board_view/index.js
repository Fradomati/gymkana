    ...
    const checkAnswer = (data) => {
        ...
        if (answer == correctAnswer) {
            ...
            updateBoard({ id: board._id, challengersToDo, challengersDone }).then(result => {
                result.status == 200 ? setBoard(result.boardUpdated) : console.log(result.message)
            })
        } else {
        ...
        <>
            {challenger
                ? (
                    ...
                    <div>
                        <div>{challenger.title}</div>
                        <div dangerouslySetInnerHTML={{ __html: challenger.description }} style={{ width: "100%" }}></div>
